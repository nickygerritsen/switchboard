...
// FindMatch finds the first matching rule for a URL and returns the browser name, profile, and whether a match was found
// If no match is found, returns the default browser with an empty profile and false for matched
func (r *Router) FindMatch(url string) (browser, profile string, matched bool) {
	...
	for i, rule := range r.config.Rules {
		...
		for _, pattern := range rule.Match {
			if matcher.Match(pattern, url) {
				logger.Info("URL %s matched rule #%d (pattern: %s) -> browser: %s, profile: %s",
					url, i+1, pattern, rule.Browser, rule.Profile)
				return rule.Browser, rule.Profile, true
			}
		}
	}
	...
	return r.config.DefaultBrowser, "", false
}
